...
export type CourseFindByIdResult = Result<Course, CourseDatabaseException>;

@Injectable()
export class CourseInfrastructure implements CourseRepository {
  async save(course: Course): Promise<CourseInsertResult> {
    try {
  ...
  async findById(id: string): Promise<CourseFindByIdResult> {
    try {
      const manager = AppService.manager;
      const coursesEntity = await manager
        .getRepository(CourseEntity)
      ...
      return ok(CourseDto.fromDataToDomain(coursesEntity));
    } catch (error) {
      return err(new CourseDatabaseException(error.message));
    }
  }
